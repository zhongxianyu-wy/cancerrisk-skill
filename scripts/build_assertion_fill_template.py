...
def build_assertion_fill_template(
    ...
    cancers = _read(store / "ontology/cancers.json")["cancers"]
    factors = _read(store / "ontology/risk_factors.json")["risk_factors"]
    assertions = _read(store / "assertions/risk_assertions.json")["assertions"]
    observables = _read(store / "ontology/factor_observables.json").get("factors", {})
    synonyms = _read(store / "ontology/factor_synonyms.json").get("synonyms", {})
    version = _read(store / "versions/evidence_version.json").get("version")
    ...
    imaging_path = store / "ontology/imaging_findings.json"
