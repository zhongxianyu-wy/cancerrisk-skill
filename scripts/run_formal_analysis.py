...
EVIDENCE_STORE = SKILL_ROOT / "evidence_store"
...
def main():
    ...
    derived_assertions = json.loads(
        (EVIDENCE_STORE / "assertions/risk_assertions_derived.json").read_text(encoding="utf-8")
    ) if (EVIDENCE_STORE / "assertions/risk_assertions_derived.json").is_file() else {"derived_assertions": []}
    ...
    try:
        evidence_version = json.loads(
            (EVIDENCE_STORE / "versions/evidence_version.json").read_text(encoding="utf-8")
        ).get("version")
