...
def build_derived_evidence(store: Path, config: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    raw_assertions = _read_json(store / "assertions/risk_assertions.json")["assertions"]
    raw_detection = _read_json(store / "assertions/detection_performance.json")["tests"]
...
def main() -> None:
    ...
    (store / "assertions/risk_assertions_derived.json").write_text(
        json.dumps(risk, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    (store / "assertions/detection_performance_derived.json").write_text(
