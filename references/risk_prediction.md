...
- All numeric updates come from `evidence_store/`. The orchestrator
  refuses to run snapshot when the evidence store is missing.
- Cancers not in `evidence_store/ontology/cancers.json` do not appear
...
- It is in `evidence_store/ontology/risk_factors.json`.
- It is in `merged_risk_factors.json.current_factor_states`.
- The combination `(cancer_id, factor_id, factor_level)` exists in
  `evidence_store/assertions/risk_assertions_derived.json` or equivalent
...
- **深入式 (deep)** — pulls from
  `evidence_store/screening/screening_recommendations.json`, filtered
...
- `cancerrisk-skill/docudatabase/<person_id>/factor_timeline.json`
  (prior factor values).
- `cancerrisk-skill/docudatabase/<person_id>/screening_test_timeline.json`
